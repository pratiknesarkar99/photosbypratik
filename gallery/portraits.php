...
<html>
	...
	<body>
		...
		<script type="text/javascript">
			function openImgjpeg(num)
			{
				window.open("./portraits/portraits"+num+".jpeg", "_current");
			}
			function openImgjpg(num)
			{
				window.open("./portraits/portraits"+num+".jpg", "_current");
			}
		</script>
		...
		<div class="container2">
			<figure>
				<div>
					<h2 style="text-align: center; color: red;"> Images in Album Portraits</h2>
					<?php
					$query = "SELECT * FROM portraits order by id DESC";
					$result = mysqli_query($connect, $query);
					while($row = mysqli_fetch_array($result)){
						$uid = uniqid();
						echo'<script><alert>$uid</alert></script>';
